...
if(mode == "naivebayes"):
	...
	topicSet = {
		"Debt Ceiling": ['debt'],
		"Fuel Efficiency": ['gas', 'natural', 'fuel', 'efficiency', 'alternative'],
		"Equal Pay": ['equal', 'pay', 'woman'],
		"Gun Control": ['gun', 'control'],
		"Abortion": ['abortion', 'pro', 'life'],
		"Welfare": ['welfare', 'social security'],
		"Obamacare": ['obamacare'],
		"Gay Marriage": ['gay', 'same sex', 'lgbtq', 'marriage'],
		"Planned Parenthood": ['planned', 'parenthood', 'funding'],
		"Global Warming": ['global', 'warming', 'climate', 'change']
	}
else:
	sys.exit("EXITING: Unrecognized mode")
...
#Helper Functions
def topicMatch(tweetText, topicSet):
	tweetTopic = None
	for topic in topicSet.keys():
		print("HAMY: Topic... " + topic)
		for word in topicSet[topic]:
			print("HAMY: Word... " + word)
...
if __name__ == "__main__":
		...
		for tweet in feed['feed']:
			...
			try:
				...
				if(mode == "naivebayes"):
					...
					tweetTopic = topicMatch(tweetText, topicSet)

				if(tweetTopic is None):
					if(debug):
						print("TWEET DROPPED: Tweet didn't fall into specified topics")
					tweetsDiscarded += 1
					continue
				elif(debug):
	...
	print("Tweets Discarded: " + str(tweetsDiscarded))
